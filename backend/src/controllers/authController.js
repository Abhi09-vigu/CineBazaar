...
export const signupOwner = async (req, res) => {
  ...
  const salt = await bcrypt.genSalt(10);
  const hash = await bcrypt.hash(password, salt);
  const user = await User.create({ name, email, password: hash, role: 'OWNER', ownerApproved: false });
  const token = signToken(user);
  return res.status(201).json({ user: { id: user._id, name: user.name, email: user.email, role: user.role, ownerApproved: user.ownerApproved }, token });
};
...
export const loginOwner = async (req, res) => {
  ...
  const ok = await bcrypt.compare(password, user.password);
  if (!ok) return res.status(401).json({ message: 'Invalid credentials' });
  if (user.role !== 'OWNER') return res.status(403).json({ message: 'Owner access required' });
  if (!user.ownerApproved) return res.status(403).json({ message: 'Owner is not approved by admin yet' });
  const token = signToken(user);
  res.json({ user: { id: user._id, name: user.name, email: user.email, role: user.role, ownerApproved: user.ownerApproved }, token });
};
